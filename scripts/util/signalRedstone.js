...
for(var l = 0; l < positions.length; l++){
    ...
    if(blockName == 'minecraft:redstone_wire'){
        world.setBlockMetadata(nx, ny, nz, 15);
    }
    ...
    if(positions[l][1] == 0){
        ...
        if(blockName == 'minecraft:unpowered_repeater'){
            if(blockMetadata % 4 == l){
                world.setBlockAndMetadata(nx, ny, nz, 'minecraft:powered_repeater', blockMetadata);
            }
        }
        // XZ隣接のレッドストーンコンパレータに信号ON
        if(blockName == 'minecraft:unpowered_comparator' && blockMetadata < 8){
            if(blockMetadata % 4 == l){
